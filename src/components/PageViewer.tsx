...
import {
  getPageInfo,
  getSelectedCategoryTitlePage
} from "src/reducers/PageViewerReducer";
    ...
    return (
      <div className="page-viewer-container">
        <div className="page-viewer-header">
          <NavLink
            to={"/Categories/" + (item ? item.Name : "")}
            className="page-viewer-back"
          >
            <h3 className="page-viewer-title">{item ? item.Title : ""}</h3>
          </NavLink>

          <img
            className="page-viewer-header-img"
            src={imageManagment.GetImageUrl(
              item && item.ImageUrl ? item.ImageUrl : "NotSetImgCourse.svg",
              "Course"
            )}
            alt="img"
          />
        ...
        <div className="page-viewer-wrapper">
          <div className="page-viewer-sidebar">
            فهرست مطالب
            <ul>{caItems}</ul>
          </div>
          ...
          <div className="page-viewer-body">
            ...
            <Content
              ...
              CategoryName={this.getPageCategory()}
              DefaultText={bodyContext}
            />
          </div>
...
const mapStateToProps = (state: IAppState, ownProps: any) => ({
  ...
  SelectedCategoryTitle: getSelectedCategoryTitlePage(
    state.pagesState,
    (ownProps as IPageViewProps).match.params.Category || ""
  )
});
